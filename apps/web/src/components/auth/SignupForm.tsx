...
export const SignupForm = () => {
  ...
  return (
    <div className="glass-panel w-full max-w-sm rounded-2xl p-5 shadow-[0_20px_60px_-30px_rgba(0,61,130,0.55)]">
      <h2 className="mb-1 text-center text-2xl font-bold text-slate-900">Create Account</h2>
      <p className="mb-5 text-center text-xs text-slate-600">Launch your first board in seconds</p>
      ...
      <form onSubmit={handleSignup} className="space-y-3">
        <div>
          <label htmlFor="displayName" className="mb-1 block text-xs font-medium text-slate-700">
            Display Name
          </label>
          <input
            id="displayName"
        ...
        <div>
          ...
          <input
            ...
            className={`w-full rounded-lg border bg-white/80 px-2.5 py-2 text-sm text-slate-900 outline-none transition-all ${
              isEmailInvalid
                ? 'border-rose-300 focus:border-rose-400 focus:ring-2 focus:ring-rose-100'
                : 'border-cyan-100 focus:border-cyan-300 focus:ring-2 focus:ring-cyan-100'
            }`}
            required
            disabled={loading}
          />
          {isEmailInvalid && (
            <p className="mt-0.5 text-xs text-rose-500">Please enter a valid email address</p>
          )}
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className="w-full rounded-lg border border-cyan-100 bg-white/80 px-2.5 py-2 pr-9 text-sm text-slate-900 outline-none transition-all focus:border-cyan-300 focus:ring-2 focus:ring-cyan-100"
              required
              disabled={loading}
              minLength={6}
            />
          ...
          <p className="mt-0.5 text-xs text-slate-500">Minimum 6 characters</p>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className={`w-full rounded-lg border bg-white/80 px-2.5 py-2 pr-9 text-sm text-slate-900 outline-none transition-all ${
                isConfirmMismatch
                  ? 'border-rose-300 focus:border-rose-400 focus:ring-2 focus:ring-rose-100'
                  : 'border-cyan-100 focus:border-cyan-300 focus:ring-2 focus:ring-cyan-100'
              }`}
          ...
          {isConfirmMismatch && (
            <p className="mt-0.5 text-xs text-rose-500">Passwords do not match</p>
          )}
        </div>
