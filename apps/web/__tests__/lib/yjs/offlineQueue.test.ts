...
describe('OfflineQueue', () => {
  ...
  it('should maintain operation order by timestamp', async () => {
    // Snapshot model: one snapshot per board; multiple saves update the same snapshot (changeCount increments)
    const ydoc = new Y.Doc();
    const ymap = ydoc.getMap('objects');
    ...
    // One snapshot per board; timestamp is from last save, changeCount reflects number of updates
    const updates = await getPendingUpdates(testBoardId);
    expect(updates).toHaveLength(1);
    expect(updates[0].boardId).toBe(testBoardId);
    expect(updates[0].changeCount).toBe(2);
  });
