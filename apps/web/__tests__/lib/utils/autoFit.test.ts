...
import { calculateBoundingBox, calculateAutoFit } from '@/lib/utils/autoFit';
import type { RectShape, CircleShape, LineShape, PathShape } from '@/types/canvas';
...
describe('calculateBoundingBox', () => {
  ...
  it('calculates bbox for path (freehand points)', () => {
    const path: PathShape = {
      id: '1',
      type: 'path',
      x: 10,
      y: 20,
      points: [0, 0, 50, 100, 120, 30],
      stroke: '#000',
      strokeWidth: 4,
      rotation: 0,
      zIndex: 1,
    ...
    expect(bbox!.maxX).toBe(130);
    expect(bbox!.maxY).toBe(120);
    expect(bbox!.width).toBe(120);
    expect(bbox!.height).toBe(100);
  });

  it('calculates bbox for path with single point (degenerate)', () => {
    const path: PathShape = {
      id: '1',
      type: 'path',
      x: 5,
    ...
    const bbox = calculateBoundingBox([path]);
    expect(bbox).not.toBeNull();
    expect(bbox!.minX).toBe(6);
    expect(bbox!.minY).toBe(6);
    expect(bbox!.maxX).toBe(6);
    expect(bbox!.maxY).toBe(6);
  });

  it('calculates bbox for multiple objects including path', () => {
    const path: PathShape = {
      id: 'p1',
      type: 'path',
      x: 0,
    ...
    const bbox = calculateBoundingBox([path, rect]);
...
describe('calculateAutoFit', () => {
  ...
  it('fits view to path-only content', () => {
    const path: PathShape = {
      id: '1',
      type: 'path',
      x: 100,
    ...
    const result = calculateAutoFit([path], 1920, 1080, 100);
    expect(result).toBeTruthy();
    expect(result!.scale).toBeGreaterThan(0.1);
    expect(result!.scale).toBeLessThanOrEqual(1);
  ...
  it('returns null for invalid viewport dimensions', () => {
    const rect: RectShape = {
      id: '1',
      type: 'rect',
      x: 0,
    ...
    expect(calculateAutoFit([rect], 0, 1080)).toBeNull();
    expect(calculateAutoFit([rect], 1920, 0)).toBeNull();
  });
});
